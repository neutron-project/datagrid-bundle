...
jQuery(document).ready(function(){  
    jQuery('.neutron-datagrid').each(function(){
        /** getting grid id */
        var datagridId = jQuery(this).attr('id');
        
        /** getting grid name */
        var gridName = datagridId.replace('neutron-datagrid-', '');
        ...
        var jqgrid = jQuery('#' + datagridId).jqGrid({
            ...
            onSelectRow: function(aRowids, status){ 
                ...
                jQuery.each(options.dependentDataGrids, function(idx, grid){
                    var subGridId = 'neutron-datagrid-' + grid;  
        ...
        jQuery('body').bind('neutron.tree.event.complete', function(e){
        ...
        if(options.massActionsEnabled){
            ...
            jQuery('#t_' + datagridId).append(
                ...
                "<input class='select-all' type='button'  value='"+ options.massActionLabels.select_all +"' style='height:20px;font-size:-3'/>" +
                "<input class='deselect-all' type='button'  value='"+ options.massActionLabels.deselect_all +"' style='height:20px;font-size:-3'/>" +
                "</div>" +
                "<div style='float:right; margin-right:15px'>" +
                "<span>"+ options.massActionLabels.with_selected +": </span> " +
                "<select class='mass-action' disabled='true'>" +
                buildSelectOptions(options.massActions, options.massActionLabels.select_action) +
                "</select>" +
                "<input class='submit-action' type='button' disabled='true' value='Submit' style='height:20px;font-size:-3'/>" +
                "</div>"
            );


            /** Assigning click event to select-all button */
            jQuery('#t_' + datagridId + ' .select-all').click(function(){ 
                jQuery('#cb_' + datagridId).attr("checked", true);
            ...
            jQuery('#t_' + datagridId + ' .deselect-all').click(function(){ 
            ...
            jQuery('#t_' + datagridId + ' .submit-action').click( function() { 
                ...
                if(ids.length){ 
            		...
            		options.mass_action_url, 
                    {
                        ...
                        var massActionEvent = jQuery.Event('neutron_datagrid.event.massAction');
                ...
                update: function(event, ui) {
                    ...
                    jQuery.post(
                        ...
                        function(response){
                            if(options.driver !== 'array'){
                                jqgrid.trigger("reloadGrid");
                                var sortableEvent = jQuery.Event('neutron_datagrid.event.sortable');
